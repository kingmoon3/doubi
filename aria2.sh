...
Download_aria2_conf(){
	mkdir "${file}" && cd "${file}"
	wget --no-check-certificate -N "https://raw.githubusercontent.com/kingmoon3/doubi/master/other/Aria2/aria2.conf"
	...
	wget --no-check-certificate -N "https://raw.githubusercontent.com/kingmoon3/doubi/master/upload.sh" && chmod +x upload.sh
	wget --no-check-certificate -N "https://raw.githubusercontent.com/kingmoon3/doubi/master/ck.sh" && chmod +x ck.sh
}
Service_aria2(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/kingmoon3/doubi/master/service/aria2_centos -O /etc/init.d/aria2; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/kingmoon3/doubi/master/service/aria2_debian -O /etc/init.d/aria2; then
			echo -e "${Error} Aria2服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/aria2
...
${Green_font_prefix}3.${Font_color_suffix} 如果要退出并保存文件，那么按 ${Green_font_prefix}ESC${Font_color_suffix} 后输入 ${Green_font_prefix}:wq${Font_color_suffix} 即可。
${Green_font_prefix}4.${Font_color_suffix} 如果要退出并不保存文件，那么按 ${Green_font_prefix}ESC${Font_color_suffix} 后输入 ${Green_font_prefix}:q!${Font_color_suffix} 即可。
...
crontab_update_start(){
	...
	echo -e "\n*/10 * * * * /bin/bash "${file}"/ck.sh" >> "$file_1/crontab.bak"
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/kingmoon3/doubi/master/aria2.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/aria2" ]]; then
		rm -rf /etc/init.d/aria2
		Service_aria2
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/kingmoon3/doubi/master/aria2.sh" && chmod +x aria2.sh
